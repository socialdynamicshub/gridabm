...
create_forest(dims = c(20, 20), tree_density = 0.7) %>% 
...
create_forest(dims = c(20, 20), tree_density = 0.3) %>% 
  plot_state(marker_size = 5, color_scheme = theme_forest_fire())
```
...
create_forest(dims = c(20, 20), tree_density = 0.9) %>% 
  plot_state(marker_size = 5, color_scheme = theme_forest_fire())
```
...
forest <- create_forest(c(20, 20), 0.7)
...
forest <- create_forest(c(20, 20), 0.5)
forest_results <- run_automaton(forest, 100, forest_fire_step)
animate_model_run(forest_results, 5, theme_forest_fire())
```
...
for (density in seq(0.1, 0.9, 0.1)) {
  for (replicate in seq(1, 5, 1)) {
    forest <- create_forest(c(20, 20), density)
    forest_results <- run_automaton(forest, 100, forest_fire_step)
...
```{r}
langtons_ant_board <- create_ant_grid(
...
```{r}
langtons_ant_results <- run_automaton(langtons_ant_board, 10000, langtons_ant_step)
langtons_ant_results %>% 
  filter(step == 10000) %>% 
  plot_state(1, theme_langtons_ant())
```
